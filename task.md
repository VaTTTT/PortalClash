...
#### Tasks:
- [ ] **Data Model & Stats Upgrade (`game.js`)**
  - [ ] Define the new `STATS` object mapping stats to the 5 tiers (`peasant`, `archer`, `swordsman`, `knight`, `wizard`).
  - [ ] Balance health, damage, speed, costs, and unlock gold parameters.
  - [ ] Introduce range capabilities:
    - [ ] Archer: Ranged attack radius (e.g. range 120-150px).
    - [ ] Wizard: Ranged magic attack (e.g. range 180-200px) with potential splash/AOE logic.
    - [ ] Melee units: Close quarters (default radius combat).
- [ ] **State Preservation & Save Compatibility**
  - [ ] Update persistent `state` initialization in `game.js` to track unlocks and upgrade levels for the 5-tier roster:
  ...
  - [ ] Implement local storage backwards compatibility (safe schema migration from older save versions).
- [ ] **UI & DOM Updates (`index.html`)**
  - [ ] Update bottom summoning bar to include 5 buttons for the 5 tiers.
  - [ ] Update Shop overlay to show unlock buttons for Archer, Swordsman, Knight, and Wizard.
  - [ ] Update Shop overlay to show upgrade buttons for all 5 units.
- [ ] **Rendering & Visual Identity**
  - [ ] Replace simple solid circle/square drawings on the canvas with character-themed visuals (e.g. drawing thematic symbols, colored shields, weapons, or using high-quality Emojis `🧑🌾`, `🏹`, `🗡️`, `🐎`, `🧙‍♂️` rendered directly inside the canvas units).
  - [ ] Render projectiles for Archer (arrows) and Wizard (magic bolts) moving towards their targets.
- [ ] **AI Behavior Upgrades**
  - [ ] Update threat detection and panic rules in AI to assess a 5-tier roster.
  - [ ] Allow Orc AI to summon all 5 unit types (Peasant Orcs, Archer Orcs, Swordsman Orcs, Knight Orcs, and Wizard Orcs) using equivalent AI behaviors.
